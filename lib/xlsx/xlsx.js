...
const VmlDrawingXform = require('./xform/drawing/vlm-drawing/vml-drawing-xform');
...
class XLSX {
  ...
  reconcile(model, options) {
    ...
    const vmlDrawingXfrom = new VmlDrawingXform();
    ...
    Object.keys(model.vmlDrawings).forEach(name => {
      const vmlDrawing = model.vmlDrawings[name];
      const vmlDrawingRel = model.vmlDrawingRels[name];
      if (vmlDrawingRel) {
        drawingOptions.rels = vmlDrawingRel.reduce((o, rel) => {
          o[rel.Id] = rel;
          return o;
        }, {});
        vmlDrawingXfrom.reconcile(vmlDrawing, drawingOptions);
      }
    });
  ...
  async _processVmlDrawingRelsEntry(entry, model, name) {
    const xform = new RelationshipsXform();
    const relationships = await xform.parseStream(entry);
    model.vmlDrawingRels[name] = relationships;
  }
      ...
      if (!entry.dir) {
        ...
        switch (entryName) {
          ...
          default: {
            ...
            match = entryName.match(/xl\/drawings\/_rels\/([a-zA-Z0-9]+)[.]vml[.]rels/);
            if (match) {
              await this._processVmlDrawingRelsEntry(stream, model, match[1]);
              break;
            }
          }
        }
  ...
  addlegacyDrawingHF(zip, model) {
    const vmlDrawingXform = new VmlDrawingXform();
    const relsXform = new RelationshipsXform();

    model.worksheets.forEach(worksheet => {
      const {legacyDrawingHF} = worksheet;
      if (legacyDrawingHF) {
        vmlDrawingXform.prepare(legacyDrawingHF, {});
        let xml = vmlDrawingXform.toXml({hfImages: worksheet.hfImages});
        zip.append(xml, {name: `xl/drawings/${legacyDrawingHF.name}.vml`});

        xml = relsXform.toXml(legacyDrawingHF.rels);
        zip.append(xml, {name: `xl/drawings/_rels/${legacyDrawingHF.name}.vml.rels`});
      }
    });
  }
  ...
  async addWorksheets(zip, model) {
    ...
    model.worksheets.forEach(worksheet => {
      ...
      if (worksheet.comments.length > 0) {
        ...
        xmlStream = new XmlStream();
        vmlDrawingXform.render(xmlStream, {...worksheet, hfImages: undefined});
  ...
  prepareModel(model, options) {
    ...
    worksheetOptions.vmlDrawings = model.vmlDrawings = [];
  ...
  async write(stream, options) {
    ...
    await this.addlegacyDrawingHF(zip, model);
