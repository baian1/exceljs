...
const LegacyDrawingHFXform = require('./legacy-drawingHF-xform');
...
class WorkSheetXform extends BaseXform {
  constructor(options) {
    ...
    this.map = {
      ...
      legacyDrawingHF: new LegacyDrawingHFXform(),
    };
  }
  ...
  prepare(model, options) {
    ...
    if (model.comments.length > 0) {
      ...
      rels.push(comment);
      const vmlDrawingIndex = ++options.vmlDrawingsCount;
      const vmlDrawing = {
        Id: nextRid(rels),
        Type: RelType.VmlDrawing,
        Target: `../drawings/vmlDrawing${vmlDrawingIndex}.vml`,
      };
      ...
      options.commentRefs.push({
        ...
        vmlDrawing: `vmlDrawing${vmlDrawingIndex}`,
      });
    }
    ...
    if (model.hfImages && model.hfImages.length > 0) {
      const rId = nextRid(rels);
      const legacyDrawingHF = (model.legacyDrawingHF = {
        rId,
        name: `vmlDrawing${++options.vmlDrawingsCount}`,
        rels: [],
      });
      const drawingHFRel = {
        Id: rId,
        Type: RelType.VmlDrawing,
        Target: `../drawings/${legacyDrawingHF.name}.vml`,
        isHFRel: true,
      };
      rels.push(drawingHFRel);
      options.vmlDrawings.push(legacyDrawingHF);
      model.hfImages.forEach(hfImage => {
        const rIdImage = nextRid(legacyDrawingHF.rels);
        const bookImage = options.media[hfImage.imagedata.index];
        hfImage.imagedata.rId = rIdImage;
        hfImage.imagedata.title = bookImage.name;
        legacyDrawingHF.rels.push({
          Id: rIdImage,
  ...
  render(xmlStream, model) {
    ...
    if (model.rels) {
      ...
      model.rels.forEach(rel => {
        if (rel.Type === RelType.VmlDrawing && !rel.isHFRel) {
          xmlStream.leafNode('legacyDrawing', {'r:id': rel.Id});
        }
      });
    }
    this.map.legacyDrawingHF.render(xmlStream, model.legacyDrawingHF);
  ...
  reconcile(model, options) {
    ...
    const rels = (model.relationships || []).reduce((h, rel) => {
      ...
      if (rel.Type === RelType.VmlDrawing && model.comments && model.comments.length) {
        const match = rel.Target.match(/\/drawings\/([a-zA-Z0-9]+)[.][a-zA-Z]{3,4}$/);
        if (match) {
          const name = match[1];
          const vmlComment = options.vmlDrawings[name].comments;
    ...
    model.hfImages = [];
    if (model.legacyDrawingHF) {
      const legacyDrawingHFRel = rels[model.legacyDrawingHF.rId];
      const match = legacyDrawingHFRel.Target.match(/\/drawings\/([a-zA-Z0-9]+)[.][a-zA-Z]{3,4}$/);
      if (match) {
        const vmlDrawingName = match[1];

        const {hfImages} = options.vmlDrawings[vmlDrawingName];
        hfImages.forEach(hfimage => {
          if (hfimage) {
            model.hfImages.push(hfimage);
          }
        });
      }
    }
